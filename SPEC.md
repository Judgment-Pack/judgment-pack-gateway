...
| the registry path exists but cannot be read, or its parent is not a directory | no verdict — the verifier refuses (non-zero exit); the anchor is present and unreadable, not absent |
