...
|---|---|---|
| `/usr/local/bin/gateway` | the core binary from `go/`: `serve`, `verify`, `canon`, `conform`, `keygen`, the engine's `connect`, and `mcp`; owned by `engine`, mode 0700, since it carries file capabilities | built from this repository at the tagged commit |
| `/usr/local/bin/engine-mcp` | the same executable, copied before the capabilities were written: root's, mode 0755, no capability; what the MCP server ([mcp-server.md](mcp-server.md)) runs as `engine-mcp` | the same build |
...
| `/etc/passwd` | the signer's user `engine` (uid 65532), the MCP server's user `engine-mcp` (uid 65533) and eight platform users `engine-1` … `engine-8` (uids 65601 … 65608), each with a home of its own alone | written at build |
...
home `/home/engine-mcp`, no supplementary group but its own, no capability). The executable is the
