...
processing is an adapter in the second module — a process that holds no seed and that the
signer never links ([ADR-0001](../adr/0001-one-engine-four-processes.md)) — and the record it
writes is a result like any other: canonicalized, retained as the artifact, its digest signed
into a version 3 receipt. **What the receipt then proves is byte lineage: these bytes, from this
program, at this call in this session, under this key.** It does not prove that the text is
what the document says, that the extraction is complete, or that a scanned page was read
right. The record states, for every page, what was done and what was not, and a consumer
reads those statements rather than assuming.
...
`"command"` (SPEC.md §1.2a), the adapter is named as configured and pinned by the digest of its
executable, every other acquisition member is `null`, and `observedAt` is the gateway's own
stamp. That is the honest shape for a document the caller supplied: there is no endpoint, no
snapshot and no peer to record, and the arguments commitment already commits to the bytes.
The record carries what the adapter can add — its own identity, when it received the bytes,
what it ran — inside the signed artifact, in the clear.
...
(`maxRequestBody`, SPEC.md §6 reference implementation), and a source runs for at most thirty
seconds. A document travels inline, base64-encoded, so until the gateway grows operator bounds
for both — `--max-request BYTES` and `--source-timeout NAME=SECONDS`, a separate change to the
core with its own review — an inline document is at most about 760 KiB and its processing, OCR
included, at most thirty seconds. The adapter's own `--max-bytes` and `--timeout` sit under
whatever the gateway allows; the record's `processing.bounds` says which bounds applied.
...
failed acquisition. What is refused: not a JSON object; a member missing, unknown, duplicated
or of another type; a name outside its rule; `bytes` empty or not base64; a `sha256` that does
not match; a request past the adapter's read bound (`--max-bytes` × 4/3 plus 64 KiB, so that a
document within the bound always fits with its name and options). Everything else — an
unsupported type, a document past `--max-bytes`, an encrypted or malformed PDF — is a
**record**, since the document's identity is established and the outcome is worth attesting.
...
exponent number anywhere, since the gateway's canonicalizer refuses those. Every member below
is present in every record; a member without a value is `null`, never absent, so that a record
made with a capability switched off never reads the same as one made before the member
existed.
...
```json
{
  ...
  "processing": {
    "status": "complete",
    "errors": [],
    "bounds": {"maxBytes": 16777216, "maxPages": 500, "maxTextBytes": 8388608, "maxInflateBytes": 67108864, "timeoutMs": 25000},
    "durationMs": 118
  },
  "provenance": {
...
| Member | Type | Meaning |
|---|---|---|
| `id` | digest | the SHA-256 of the original bytes, as `"sha256:"` + 64 lowercase hex. **This is the document's identity**: two attachments of the same bytes have the same `id` whatever they were named, and a consumer that holds the bytes can recompute it. It is the same digest the caller may have claimed in `document.sha256` |
...
| `extraction` | `"text-layer"`, `"ocr"`, `"mixed"`, `"verbatim"` or `"none"` | how the document's text was obtained, over all pages: from the PDF's own text layer; from the OCR program; from both, on different pages; verbatim, for a text document; or not at all |
...
| `truncated` | boolean | `true` when pages exist that were not processed — past the page bound, past the text bound, or past the deadline. The pages listed are complete in themselves; what follows them is not in the record |
| `chars` | integer | the sum of the pages' `chars` |
| `pages` | array of page objects | in page order, one per processed page, never more than `maxPages` |
...
| `status` | `"ok"`, `"no-text"`, `"needs-ocr"` or `"failed"` | `"ok"`: text was obtained; `"no-text"`: the page has neither text operators nor images — blank, or drawn with paths alone; `"needs-ocr"`: the page has images and no text layer, and no OCR text was applied — a scanned page; `"failed"`: the page's content could not be interpreted, and `processing.errors` names why with this page's number |
| `extraction` | `"text-layer"`, `"ocr"`, `"verbatim"` or `"none"` | how this page's text was obtained: from the PDF's text layer, from the OCR program, verbatim for a text document, or not at all |
| `text` | string | the page's text; `""` when there is none. Lines end with `\n`; a page's text has no leading or trailing blank lines. Glyphs the adapter could not map to Unicode are U+FFFD, and counted |
...
Text from OCR is what the program answered, unchanged. For a text document the text is the
bytes themselves, as one page, with `\r\n` and `\r` normalised to `\n`.

**Page references.** A citation into an attachment names the document by `id`, the page by
`number`, and the excerpt as text matched against that page's `text` — never by offset into a
concatenation the record does not carry. Offsets within a page's `text` are the consumer's own,
in whatever unit it counts; the record gives `chars` so a consumer can check it read the whole
page.
...
| Member | Type | Meaning |
|---|---|---|
| `status` | `"complete"`, `"partial"` or `"failed"` | `"complete"`: every page is `"ok"` or `"no-text"`, nothing was truncated and there are no errors. `"partial"`: the document was read, and not all of it became content — a page needs OCR, a page failed, pages were cut by a bound or the deadline. `"failed"`: no content could be produced because the document could not be read: unsupported or mismatched type, past the size bound, encrypted and not openable, malformed past recovery. A failed record has `content.pages` empty, `extraction` `"none"` |
| `errors` | array of error objects | why the status is not `"complete"`, one entry per condition, in the order they were met; empty for a complete record |
| `bounds` | object | the bounds that applied: `maxBytes`, `maxPages`, `maxTextBytes`, `maxInflateBytes`, `timeoutMs`, each an integer. A consumer that finds `truncated` reads these to know what to raise |
...
for a person, at most 512 bytes, and never carries the document's content; `page` is the page
the condition belongs to, or `null` for the document as a whole. The codes of version 1:
...
| `document-over-bound` | failed | the document exceeds `maxBytes`; nothing past the size check was done |
...
| `text-over-bound` | partial | the extracted text reached `maxTextBytes`; the page on which it did is cut and later pages were not processed |
...
| `ocr-not-run` | partial | pages need OCR and none ran: no program is configured, or the caller asked `"never"`. The message says which |
| `ocr-failed` | partial | the OCR program exited with a status, wrote something that is not its contract, or named a page twice or out of range; the pages it was asked for stay `"needs-ocr"` |
...
| Member | Type | Meaning |
|---|---|---|
| `adapter` | `{"name": string, "version": string, "digest": digest}` | the adapter as it identifies itself: its name, its version, and the SHA-256 of its own executable, read at run time — the same identity the receipt's `acquisition.adapter` carries for a bare command, where the name is the command as the operator configured it |
| `source` | object | where the bytes came from: `{"kind": "inline"}` for a document the caller supplied. Retrieval from a drive adds its own kind with its members; the members present depend on `kind`, and a consumer reads `kind` first |
| `observedAt` | string | when the adapter had read the request in full, `YYYY-MM-DDThh:mm:ssZ`, UTC, whole seconds — the form `servedAt` takes. The receipt's own `observedAt` for a bare command is the gateway's stamp of when it read the output, which is later |
| `processor` | string or `null` | the extraction implementation and its algorithm version: `"adapter-document/pdf/1"`, `"adapter-document/text/1"`. It moves when the extractor's output for the same bytes changes, so two records of one document under one `processor` and one OCR program are comparable, and two under different ones are not. `null` when no extractor ran: an unsupported or mismatched type, a document past the size bound |
| `ocr` | object or `null` | `{"program": string, "digest": digest, "pages": array of integers}` when the OCR program ran and its answer was applied: the program as configured, the SHA-256 of the executable that name resolved to, and the pages whose text came from it, ascending. The digest pins the program named, not what it in turn runs. `null` when no OCR text was applied |
...
- It is run once per document, with the 1-based numbers of the pages to read as its arguments,
  ...
  which the gateway declared (`--source-env`) and is otherwise empty.
- It writes one JSON object on stdout: `{"pages": [{"number": integer, "text": string}, …]}`.
  Each number once, each within the document; a page it was not asked for is ignored. Anything
  else — another shape, a page twice, a page out of range, invalid UTF-8, output past the text
  bound, a non-zero exit — is `ocr-failed`, and no page it answered is applied.
- It has the time left under the adapter's deadline and no more; at the deadline it is killed,
  with its process group, and the outcome is `ocr-timeout`.
- Its text is applied to the pages it was asked for, as answered, with `extraction` `"ocr"`.
...
| Flag | Default | Bounds |
|---|---|---|
| `--max-bytes` | 16 MiB | the decoded document; past it, `document-over-bound` |
| `--max-pages` | 500 | pages processed; past it, `pdf-pages-over-bound` and `truncated` |
| `--max-text` | 8 MiB | extracted text over all pages, in bytes of UTF-8; past it, `text-over-bound` and `truncated` |
...
the OCR program at it, and writes what it has. The gateway kills the source group at its own
thirty seconds without waiting for anything, so an adapter deadline under the gateway's is
what makes a partial record possible; an adapter that runs into the gateway's kill has minted
nothing. A caller that gives up on a call cannot cancel the source through `/acquire`: the
call completes or times out on its own, and a receipt may be minted for a document the caller
no longer wants. That is the gateway's contract for every source, and the desk should hold a
document call to its own budget before making it.

## Text documents

`text/plain`, `text/markdown`, `text/csv` and `application/json` are carried as one page of
`"verbatim"` text: the bytes, required to be valid UTF-8, with `\r\n` and `\r` normalised to
`\n` and a leading UTF-8 byte-order mark dropped. Nothing is parsed — JSON is not
canonicalized, CSV is not split — so the text is the file. The same bounds apply; the only
failure a text document has past its type is `media-type-mismatch` on invalid UTF-8.
...
1. **Verify the receipt first**, as for any acquisition. Its `acquisition.shape` is
   ...
   command as the gateway was configured, and its `digest` pins the executable. Keep the
   `args` salt beside the record if the desk wants to show, later, exactly which bytes it
   submitted: the arguments commitment is the only thing in the receipt that binds them.
2. **Hold the record to `attachmentVersion` `"1"`** and to this note's shapes before reading
   further; a record of another version is not this contract.
3. **Read `processing.status` before `content`.** A `"failed"` record is an attachment that
   ...
   are `"no-text"`.
4. **Attach by `document.id`.** Two uploads of the same file are one document with two names;
   a re-processing of the same bytes under the same `processor` should give the same pages.
5. **Cite by page.** An excerpt names `document.id`, the page `number` and the text as it
   appears in that page's `text`, matched exactly or with whitespace folded.
...
| `mixed.pdf` | one text page and one image-only page | partial, `mixed` once an OCR program answers, else `ocr-not-run` |
...
under a `--max-bytes` smaller than it.

## Versioning

`attachmentVersion` names this contract. Within version `"1"`:

- a member is never removed, renamed or retyped, and an enumeration never loses a value;
- a new member, or a new value in an enumeration, may be added, and is recorded in this note's
  changelog with the adapter version that first writes it. A consumer must tolerate a member
  it does not know at any depth, and read an enumeration value it does not know as the class's
  unknown (an unknown `status` is not `"complete"`; an unknown error code is an error);
- the meaning of an existing member never changes.
