...
| `encryption` | object or `null` | `{"handler": string or null, "revision": integer or null, "opened": boolean}` when the adapter read a trailer that names an encryption dictionary: the dictionary's `/Filter` name as the document declares it, or `null` when that is absent, not a name, or a name whose bytes are not valid UTF-8, or when the deadline passed before the adapter read it; its `/R` as declared when that is an integer from −(2^53 − 1) to 2^53 − 1, the canonical domain's range, and `null` when it is absent, not an integer (a real such as `4.0` included), or outside that range, or when the deadline passed before the adapter read it — a record whose field is `null` for that reason carries the `timeout`; and whether the adapter opened the document. Version 1 opens only handler `Standard` at revisions 2 to 6 (RC4 and AES) with an empty user password; a document that needs a password, declares another handler or revision, or whose encryption dictionary cannot be read is not opened and fails with `pdf-encrypted`. A document whose encryption dictionary the deadline stopped the adapter reading, before its encryption was established, is not opened either, and fails with `timeout` alone, no page counted, and not `pdf-encrypted`: nothing was found that the adapter cannot open. The adapter reads the dictionary's fields in one order and decides each failure as soon as the field that establishes it has been read — a handler other than `Standard` once `/Filter` is read, a revision it does not implement once `/R` is — so a failure met before the deadline stands, and is `pdf-encrypted` whenever the deadline passes after it. The record describes the encryption dictionary as last read in full, which may be the one under the cross-reference the adapter first read if a rebuild was stopped before reading its own: a reading is in full when it opened the dictionary or met such a failure, and a reading the cross-reference was rebuilt under before it finished counts for nothing, the rebuild reading the dictionary its own trailer names. So a dictionary that a rebuild of the cross-reference read and opened before the deadline stopped the rebuild is declared as it was read and `opened` `true`, with the `timeout` and no page counted; and where no reading was in full before the deadline, the record declares what the last reading had read when the deadline stopped it, `opened` `false`. `null` when the adapter read a trailer that names no encryption dictionary, and also when it did not get as far as a trailer — a mismatched or unsupported declaration, or a PDF that failed as `pdf-malformed` before its trailer was read — so `null` says no encryption was found, not that none exists |
...
4. **A PDF is opened and its pages counted.** The adapter reads the cross-reference, the trailer
   ...
   - **at the deadline**: `pageCount` is the number of pages found so far, `timeout` is recorded,
     ...
     `pdf-encrypted` or under this `timeout`, never both — and where it was established, a
     rebuild's opening of it included, `encryption.opened` is `true`. The record describes the
     encryption dictionary as last read in full, which may be the one under the cross-reference
     the adapter first read if a rebuild was stopped before reading its own. A failure met
     reading the dictionary, decided as soon as the field that establishes it has been read, is
     not undone by a deadline that passes after it: that document fails with `pdf-encrypted`;
