...
4. **A PDF is opened and its pages counted.** The adapter reads the cross-reference, the trailer
   ...
   - **at the deadline**: `pageCount` is the number of pages found so far, `timeout` is recorded,
     ...
     rebuild's opening of it included, `encryption.opened` is `true`. A failure met reading the
     dictionary before the deadline is not undone by it: that document fails with
     `pdf-encrypted`;
