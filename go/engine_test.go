...
func TestEngineConfigRefusals(t *testing.T) {
	...
	cases := []struct {
		...
		{"identity present", engineJSON(t, catalog, `,"identity":{"issuer":"https://login.example.com/"}`, good), "identity is not supported by this release"},
...
func TestBindingRefusals(t *testing.T) {
	cases := map[string]string{
		...
		`{"bindingVersion":"1","platform":"p","operations":{"live":{"shape":"mcp","server":{"image":"x/mcp@` + testImageDigest + `"},"tools":["q"],"probe":"other","licence":"MIT"}}}`: `probe "other" is not one of its tools`,
		`{"bindingVersion":"1","platform":"p","operations":{"live":{"shape":"mcp","server":{"image":"x/mcp@` + testImageDigest + `"},"tools":["q"],"probe":"","licence":"MIT"}}}`:      "probe, when present, names a tool",
