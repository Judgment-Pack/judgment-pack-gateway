...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if fs.NArg() != 0 || *image == "" || *credentials == "" {
		fmt.Fprintln(stderr, "usage: adapter-airbyte --image REF@sha256:HEX --credentials FILE [--runtime docker|podman] [--endpoint HOST] [--max-records N] [--max-output BYTES] [--timeout D]")
		return 2
	}
	...
	defer cancel()
	out, err := airbyte.Acquire(ctx, airbyte.Config{
