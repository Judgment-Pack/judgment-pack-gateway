...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	// A flag diagnostic is the operator's own command line quoted back, and
	// it is bounded before it is written: a credential belongs in the
	// credentials file and never on this line, and the redactor cannot yet
	// know one, since the file's path is what the flags name. The listing of
	// this command's own flags quotes nothing of that line, so it goes to
	// stderr whole, after the diagnostic it follows: the guidance each flag
	// carries is what an operator asking how to run the command came for,
	// and a bound on the listing cut it off at the fourth flag.
	var flagOut bytes.Buffer
	fs.SetOutput(&flagOut)
	writeFlagDiagnostic := func() {
		if flagOut.Len() > 0 {
			fmt.Fprint(stderr, redact.Diagnostic(flagOut.String(), false, nil))
			flagOut.Reset()
		}
	}
	defer writeFlagDiagnostic()
	fs.Usage = func() {
		writeFlagDiagnostic()
		fmt.Fprintln(stderr, "Usage of adapter-http:")
		fs.SetOutput(stderr)
		fs.PrintDefaults()
		fs.SetOutput(&flagOut)
	}
	...
	timeout := fs.Duration("timeout", 20*time.Second, "time allowed for the request. The gateway's source timeout (thirty seconds by default, or the --source-timeout given that source) starts before the adapter does, so keep this under it, with further margin for that start and for the adapter's report: the margin is what a request reaching this deadline needs to be reported rather than killed")
